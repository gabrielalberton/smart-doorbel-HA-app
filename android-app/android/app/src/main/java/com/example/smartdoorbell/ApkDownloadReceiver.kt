...
class ApkDownloadReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (completedId >= 0) tryHandle(context, completedId, launchInstaller = false)
    }
    ...
    companion object {
        ...
        /** Returns true when polling should stop: terminal result or another download replaced this ID. */
        fun tryHandle(context: Context, completedId: Long, launchInstaller: Boolean): Boolean {
            if (completedId != ApkDownloadSupport.pendingDownloadId(context)) return true
            ...
            if (status != DownloadManager.STATUS_SUCCESSFUL) {
                ...
                Toast.makeText(context, "Falha ao baixar a atualização (código $reason)", Toast.LENGTH_LONG).show()
                return true
            }
            ...
            if (launchInstaller) runCatching { context.startActivity(installIntent) }
            Toast.makeText(
                context,
                if (launchInstaller) "Download concluído. Abrindo instalador…"
                else "Download concluído. Toque em “Atualização da Campainha pronta” para instalar.",
                Toast.LENGTH_LONG
            ).show()
            return true
        }
